...
Write ONE reply in the ${styleDisplayName} style. Just the reply, nothing else. Max 2 sentences, under 20 words. No em dash (—), no en dash (–), no spaced hyphen as a pause; commas are fine. Sound like a real human texting. Slang and 1-2 emojis are okay if it fits.${ideaBlock}`
}
...
/** Retries once; returns '' on failure so other styles can still return (Promise.allSettled). */
async function generateOneStyleWithRetry(args) {
  const attempts = 2
  for (let i = 0; i < attempts; i++) {
    try {
      const t = await generateOneStyle(args)
      if (t) return t
    } catch (e) {
      if (i === attempts - 1) console.error('[generateOneStyle]', args.styleKey, e)
    }
  }
  return ''
}
